...
export type Lang = "ar" | "en" | "fr" | "es" | "de" | "zh";
...
const en = {
  ...
  description:
    "A smart app that turns noisy data into deep insights and measures the real impact of your campaigns.",
  ...
  exec_summary:
    "The {name} campaign scored {score}/100 with an engagement rate of {er}%, classified as {grade}. Audience response was predominantly positive ({pos}%), with peak activity between {from} and {to}.",
...
const ar: Dict = {
  ...
  exec_summary:
    "حصلت حملة {name} على {score}/100 بمعدل تفاعل {er}%، مصنّفة كـ{grade}. كانت استجابة الجمهور إيجابية إلى حدّ كبير ({pos}%)، وكان وقت الذروة بين {from} و{to}.",
...
const fr: Dict = {
  ...
  description:
    "Une app intelligente qui transforme les données bruyantes en insights et mesure l'impact réel de vos campagnes.",
  ...
  exec_summary:
    "La campagne {name} a obtenu {score}/100 avec un engagement de {er}%, classée {grade}. Réaction majoritairement positive ({pos}%), pic entre {from} et {to}.",
...
const dicts: Record<Lang, Dict> = { en, ar, fr, es, de, zh };
...
export function I18nProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const stored = (typeof window !== "undefined" &&
      localStorage.getItem("pi-lang")) as Lang | null;
  ...
  const value = useMemo<Ctx>(() => {
    ...
    return {
      ...
      dir: lang === "ar" ? "rtl" : "ltr",
      t: (k, vars) => {
        let s = (dict[k] ?? en[k] ?? String(k)) as string;
        if (vars)
